...
def association(
    w: torch.tensor, attribute_a: torch.tensor, attribute_b: torch.tensor
) -> float:
...
def weat_effect_size(
    ...
    assoc_x = [association(x, attribute_a, attribute_b) for x in target_x]
    assoc_y = [association(y, attribute_a, attribute_b) for y in target_y]
    assoc_xy = [association(xy, attribute_a, attribute_b) for xy in target_xy]
...
def main():
    ...
    x_repr, y_repr, a_repr, b_repr = [
        get_representation(dat, tokenizer, model)
        for dat in [target_x, target_y, attribute_a, attribute_b]
    ]
